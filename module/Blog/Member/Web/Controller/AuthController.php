    ...
    public function register(Request $request)
    {
        ...
        if ($registerType === 'expert') {
            ...
            if (!empty($sports) && is_array($sports)) {
                \Module\Member\Util\MemberMetaUtil::set($member->id, 'favorite_sports', json_encode($sports));
            }

            // 保存选择的大神库到 member_meta
            $expertLibraries = $request->input('expert_libraries');
            if (!empty($expertLibraries) && is_array($expertLibraries)) {
                \Module\Member\Util\MemberMetaUtil::set($member->id, 'expert_libraries', json_encode($expertLibraries));
            }
            ...
            if (!empty($certFiles)) {
                foreach ($certFiles as $certFile) {
                    if ($certFile->isValid()) {
                        ...
                        // 记录证书信息 (存储路径和原始文件名)
                        $uploadedCertsInfo[] = [
                            'path' => \Illuminate\Support\Facades\Storage::url($path), // 获取可公开访问的URL
            ...
            if (!empty($uploadedCertsInfo)) {
                 \Module\Member\Util\MemberMetaUtil::set($member->id, 'certs', json_encode($uploadedCertsInfo));
            }
        }
