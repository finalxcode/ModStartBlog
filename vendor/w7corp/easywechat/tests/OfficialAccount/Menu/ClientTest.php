...
class ClientTest extends TestCase
{
    ...
    public function testCreate()
    {
        $client = $this->mockApiClient(Client::class);
        // with match rule
        $client->expects()->httpPostJson('cgi-bin/menu/addconditional', ['button' => ['foo' => 'bar'], 'matchrule' => ['tag_id' => 1]])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->create(['foo' => 'bar'], ['tag_id' => 1]));
        // without match rule
        $client->expects()->httpPostJson('cgi-bin/menu/create', ['button' => ['foo' => 'bar']])->andReturn('mock-result');
