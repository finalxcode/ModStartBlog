...
class ClientTest extends TestCase
{
    public function testIdCard()
    {
        ...
        $path = '/foo/bar.jpg';
        $client->expects()->httpPost('cv/ocr/idcard', ['type' => 'photo', 'img_url' => $path])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->idCard($path, 'photo'));
        $client->expects()->httpPost('cv/ocr/idcard', ['type' => 'scan', 'img_url' => $path])->andReturn('mock-result');
    ...
    public function testBankCard()
    {
        $client = $this->mockApiClient(Client::class);
        $path = '/foo/bar.jpg';
        $client->expects()->httpPost('cv/ocr/bankcard', ['img_url' => $path])->andReturn('mock-result');
    ...
    public function testVehicleLicense()
    {
        $client = $this->mockApiClient(Client::class);
        $path = '/foo/bar.jpg';
        $client->expects()->httpPost('cv/ocr/drivinglicense', ['img_url' => $path])->andReturn('mock-result');
