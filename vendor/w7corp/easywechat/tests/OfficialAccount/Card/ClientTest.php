...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $attributes = ['base_info' => ['brand_name' => '微信餐厅', 'code_type' => 'CODE_TYPE_TEXT', 'title' => '132元双人火锅套餐', 'use_limit' => 100, 'get_limit' => 3], 'advanced_info' => ['use_condition' => ['accept_category' => '鞋类', 'reject_category' => '阿迪达斯', 'can_use_with_other_discount' => true]]];
        $client->expects()->httpPostJson('card/create', ['card' => ['card_type' => 'MEMBER_CARD', 'member_card' => $attributes]])->andReturn('mock-result');
    ...
    public function testGetQrCode()
    {
        ...
        $this->assertSame(['status' => $response->getStatusCode(), 'reason' => $response->getReasonPhrase(), 'headers' => $response->getHeaders(), 'body' => strval($response->getBody()), 'url' => $baseUri . '?' . http_build_query($params)], $client->getQrCode('mock-ticket'));
    }
    ...
    public function testCreateLandingPage()
    {
        ...
        $scene = 'mock-scene';
        $cardList = ['foo', 'bar'];
        $client->expects()->httpPostJson('card/landingpage/create', ['banner' => $banner, 'page_title' => $pageTitle, 'can_share' => $canShare, 'scene' => $scene, 'card_list' => $cardList])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('card/batchget', ['offset' => 0, 'count' => 10, 'status_list' => 'CARD_STATUS_VERIFY_OK'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list());
        $client->expects()->httpPostJson('card/batchget', ['offset' => 1, 'count' => 10, 'status_list' => 'CARD_STATUS_VERIFY_OK'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list(1));
        $client->expects()->httpPostJson('card/batchget', ['offset' => 1, 'count' => 10, 'status_list' => 'CARD_STATUS_VERIFY_OK'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list(1, 10));
        $client->expects()->httpPostJson('card/batchget', ['offset' => 1, 'count' => 10, 'status_list' => 'CUSTOM'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list(1, 10, 'CUSTOM'));
    }
    public function testUpdate()
    {
        $client = $this->mockApiClient(Client::class);
        $attributes = ['base_info' => [
            'brand_name' => '微信餐厅',
            ...
            'use_limit' => 100,
            'get_limit' => 3,
        ], 'advanced_info' => ['use_condition' => ['accept_category' => '鞋类', 'reject_category' => '阿迪达斯', 'can_use_with_other_discount' => true]]];
        // case 1
        $client->expects()->httpPostJson('card/update', ['card_id' => 'mock-card-id', 'member_card' => []])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->update('mock-card-id', 'member_card'));
        // case 2
        $client->expects()->httpPostJson('card/update', ['card_id' => 'mock-card-id', 'member_card' => $attributes])->andReturn('mock-result');
    ...
    public function testUpdateStock()
    {
        ...
        $client->expects()->httpPostJson('card/modifystock', ['card_id' => 'mock-card-id', 'increase_stock_value' => 10])->andReturn('mock-result')->twice();
        $this->assertSame('mock-result', $client->updateStock('mock-card-id', 10));
        $this->assertSame('mock-result', $client->updateStock('mock-card-id', -10));
        $client->expects()->httpPostJson('card/modifystock', ['card_id' => 'mock-card-id', 'reduce_stock_value' => 10])->andReturn('mock-result')->twice();
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $attributes = [['card_id' => 'mock-card-id', 'is_open' => true, 'need_verify_cod' => false, 'need_remark_amount' => false], ['card_id' => 'mock-card-id', 'is_open' => true, 'need_verify_cod' => false, 'need_remark_amount' => true], ['card_id' => 'mock-card-id', 'is_open' => true, 'need_verify_cod' => true, 'need_remark_amount' => false], ['card_id' => 'mock-card-id', 'is_open' => true, 'need_verify_cod' => true, 'need_remark_amount' => true], ['card_id' => 'mock-card-id', 'is_open' => false, 'need_verify_cod' => false, 'need_remark_amount' => false], ['card_id' => 'mock-card-id', 'is_open' => false, 'need_verify_cod' => false, 'need_remark_amount' => true], ['card_id' => 'mock-card-id', 'is_open' => false, 'need_verify_cod' => true, 'need_remark_amount' => false], ['card_id' => 'mock-card-id', 'is_open' => false, 'need_verify_cod' => true, 'need_remark_amount' => true]];
        foreach ($attributes as $attribute) {
            $client->expects()->httpPostJson('card/selfconsumecell/set', $attribute)->andReturn('mock-result');
            $this->assertSame('mock-result', $client->setPayConsumeCell($attribute['card_id'], $attribute['is_open'], $attribute['need_verify_cod'], $attribute['need_remark_amount']));
        }
    }
}