...
class JssdkClientTest extends TestCase
{
    ...
    {
        $app = new ServiceContainer(['app_id' => '123456']);
        $client = $this->mockApiClient(JssdkClient::class, ['getCache'], $app);
        $cache = \Mockery::mock(CacheInterface::class);
        $ticket = ['ticket' => 'mock-ticket', 'expires_in' => 7200];
    ...
    public function testAssign()
    {
        ...
        $cards = [['card_id' => 'mock-card-id1'], ['card_id' => 'mock-card-id2']];
        $client->expects()->attachExtension('mock-card-id1', ['card_id' => 'mock-card-id1'])->andReturn(['card_id' => 'mock-card-id1', 'assigned' => 'yes']);
        $client->expects()->attachExtension('mock-card-id2', ['card_id' => 'mock-card-id2'])->andReturn(['card_id' => 'mock-card-id2', 'assigned' => 'yes']);
        $this->assertSame(json_encode([['card_id' => 'mock-card-id1', 'assigned' => 'yes'], ['card_id' => 'mock-card-id2', 'assigned' => 'yes']]), $client->assign($cards));
    }
    public function testAttachExtension()
    {
        $client = $this->mockApiClient(JssdkClient::class, ['dictionaryOrderSignature', 'getTicket']);
        $card = ['card_id' => 'mock-card-id1', 'code' => 'mock-code', 'openid' => 'mock-openid', 'outer_id' => 'mock-outer_id', 'balance' => 'mock-balance', 'fixed_begintimestamp' => 'mock-fixed_begintimestamp', 'outer_str' => 'mock-outer_str'];
        $client->expects()->dictionaryOrderSignature('mock-ticket', \Mockery::type('int'), 'mock-card-id', 'mock-code', 'mock-openid', \Mockery::type('string'))->andReturn('mock-signature');
        ...
        $this->assertArraySubset(['code' => 'mock-code', 'openid' => 'mock-openid', 'outer_id' => 'mock-outer_id', 'balance' => 'mock-balance', 'fixed_begintimestamp' => 'mock-fixed_begintimestamp', 'outer_str' => 'mock-outer_str', 'signature' => 'mock-signature'], $ext);
    }
}