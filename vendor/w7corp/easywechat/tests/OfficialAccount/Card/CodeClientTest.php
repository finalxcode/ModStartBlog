...
class CodeClientTest extends TestCase
{
    public function testDeposit()
    {
        ...
        $cardId = 'mock-card-id';
        $params = ['card_id' => $cardId, 'code' => ['code1', 'code2']];
    ...
    public function testGet()
    {
        ...
        $cardId = 'mock-card-id';
        $code = 'mock-code';
        $client->expects()->httpPostJson('card/code/get', ['code' => $code, 'check_consume' => true, 'card_id' => $cardId])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->get($code, $cardId));
        $client->expects()->httpPostJson('card/code/get', ['code' => $code, 'check_consume' => false, 'card_id' => $cardId])->andReturn('mock-result');
