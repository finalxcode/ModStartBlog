...
class CoinClientTest extends TestCase
{
    ...
    public function testGetPrice()
    {
        ...
        $cardId = 'mock-card-id';
        $quantity = 100;
        $params = ['card_id' => $cardId, 'quantity' => $quantity];
    ...
    {
        $client = $this->mockApiClient(CoinClient::class);
        $client->expects()->httpPostJson('card/pay/recharge', ['coin_count' => 100])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->recharge(100));
    }
    public function testOrder()
    {
        $client = $this->mockApiClient(CoinClient::class);
        $client->expects()->httpPostJson('card/pay/getorder', ['order_id' => 'mock-order-id'])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(CoinClient::class);
        $client->expects()->httpPostJson('card/pay/confirm', ['card_id' => 'mock-card-id', 'order_id' => 'mock-order-id', 'quantity' => 20])->andReturn('mock-result');
