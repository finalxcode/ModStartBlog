...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('scan/product/v2/status', ['status_ticket' => 'mock-ticket'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->status('mock-ticket'));
    }
    public function testGet()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('scan/product/v2/getinfo', ['product' => ['pid' => 'mock-pid']])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->get('mock-pid'));
    }
    public function testList()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('scan/product/v2/getinfobypage', ['page_context' => 'mock-context', 'page_num' => 1, 'page_size' => 10])->andReturn('mock-result');
