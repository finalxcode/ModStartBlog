...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('customservice/kfaccount/add', ['kf_account' => 'overtrue@test', 'nickname' => '小超'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->create('overtrue@test', '小超'));
    }
    public function testUpdate()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('customservice/kfaccount/update', ['kf_account' => 'overtrue@test', 'nickname' => '小小超'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->update('overtrue@test', '小小超'));
    }
    public function testDelete()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('customservice/kfaccount/del', [], ['kf_account' => 'overtrue@test'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->delete('overtrue@test'));
    }
    public function testInvite()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('customservice/kfaccount/inviteworker', ['kf_account' => 'overtrue@test', 'invite_wx' => 'notovertrue'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->invite('overtrue@test', 'notovertrue'));
    }
    public function testSetAvatar()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpUpload('customservice/kfaccount/uploadheadimg', ['media' => '/path/to/image.jpg'], [], ['kf_account' => 'overtrue@test'])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/message/custom/typing', ['touser' => 'open-id', 'command' => 'Typing'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->showTypingStatusToUser('open-id'));
    }
    public function testHideTypingStatusToUser()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/message/custom/typing', ['touser' => 'open-id', 'command' => 'CancelTyping'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->hideTypingStatusToUser('open-id'));
    }
    public function testMessages()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('customservice/msgrecord/getmsglist', ['starttime' => 1464710400, 'endtime' => 1464796800, 'msgid' => 1, 'number' => 10000])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->messages(1464710400, 1464796800));
        $client->expects()->httpPostJson('customservice/msgrecord/getmsglist', ['starttime' => 1464710400, 'endtime' => 1464796800, 'msgid' => 2, 'number' => 100])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->messages(1464710400, 1464796800, 2, 100));
        // time
        $client->expects()->httpPostJson('customservice/msgrecord/getmsglist', ['starttime' => strtotime('2017-08-05 12:00:00'), 'endtime' => strtotime('2017-08-05 12:01:00'), 'msgid' => 2, 'number' => 100])->andReturn('mock-result');
