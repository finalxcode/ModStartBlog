...
class MessengerTest extends TestCase
{
    ...
    public function testSend()
    {
        ...
        // without by
        $client->expects()->send(['touser' => 'mock-openid', 'msgtype' => 'text', 'text' => ['content' => 'text message.']]);
        $messenger = new Messenger($client);
        $messenger->message('text message.')->to('mock-openid');
        $messenger->send();
        // with from
        $client->expects()->send(['touser' => 'mock-openid', 'msgtype' => 'text', 'customservice' => ['kf_account' => 'overtrue@test'], 'text' => ['content' => 'text message.']]);
        $messenger = new Messenger($client);
    ...
    {
        $client = \Mockery::mock(Client::class);
        $message = new Raw(json_encode(['touser' => 'mock-openid', 'msgtype' => 'text', 'text' => ['content' => 'text message.']]));
        $client->expects()->send(['touser' => 'mock-openid', 'msgtype' => 'text', 'text' => ['content' => 'text message.']]);
        $messenger = new Messenger($client);
