...
class SessionClientTest extends TestCase
{
    public function testList()
    {
        ...
        $client->expects()->httpGet('customservice/kfsession/getsessionlist', ['kf_account' => 'overtrue@test'])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(SessionClient::class);
        $client->expects()->httpPostJson('customservice/kfsession/create', ['kf_account' => 'overtrue@test', 'openid' => 'mock-openid'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->create('overtrue@test', 'mock-openid'));
    }
    public function testClose()
    {
        $client = $this->mockApiClient(SessionClient::class);
        $client->expects()->httpPostJson('customservice/kfsession/close', ['kf_account' => 'overtrue@test', 'openid' => 'mock-openid'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->close('overtrue@test', 'mock-openid'));
    }
    public function testGet()
    {
        $client = $this->mockApiClient(SessionClient::class);
        $client->expects()->httpGet('customservice/kfsession/getsession', ['openid' => 'mock-openid'])->andReturn('mock-result');
