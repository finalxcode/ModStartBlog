...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/search_map_poi', ['districtid' => 2, 'keyword' => '北京'])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/apply_merchant', ['foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->createMerchant(['foo' => 'bar']));
    }
    public function testUpdateMerchant()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/modify_merchant', ['foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->updateMerchant(['foo' => 'bar']));
    }
    public function testCreateFromMap()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/create_map_poi', ['foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->createFromMap(['foo' => 'bar']));
    }
    public function testCreate()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/add_store', ['foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->create(['foo' => 'bar']));
    }
    public function testUpdate()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/update_store', ['foo' => 'bar', 'poi_id' => 246])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/get_store_list', ['offset' => 0, 'limit' => 10])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list());
        $client->expects()->httpPostJson('wxa/get_store_list', ['offset' => 1, 'limit' => 20])->andReturn('mock-result');
