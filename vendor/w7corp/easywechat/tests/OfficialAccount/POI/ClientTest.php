...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/poi/getpoilist', ['begin' => 0, 'limit' => 10])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list());
        $client->expects()->httpPostJson('cgi-bin/poi/getpoilist', ['begin' => 1, 'limit' => 20])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list(1, 20));
    }
    public function testCreate()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/poi/addpoi', ['business' => ['base_info' => ['foo' => 'bar']]])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/poi/updatepoi', ['business' => ['base_info' => ['foo' => 'bar', 'poi_id' => 246]]])->andReturn('mock-result');
