...
class CardClientTest extends TestCase
{
    public function testSet()
    {
        ...
        $client->expects()->httpPostJson('bizwifi/couponput/set', ['foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->set(['foo' => 'bar']));
    }
    public function testGet()
    {
        $client = $this->mockApiClient(CardClient::class);
        $client->expects()->httpPostJson('bizwifi/couponput/get', ['shop_id' => 100])->andReturn('mock-result');
