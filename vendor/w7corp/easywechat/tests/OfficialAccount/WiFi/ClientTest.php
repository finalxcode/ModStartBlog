...
class ClientTest extends TestCase
{
    public function testSummary()
    {
        ...
        $client->expects()->httpPostJson('bizwifi/statistics/list', ['begin_date' => 'mock-begin-date', 'end_date' => 'mock-end-date', 'shop_id' => 100])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->summary('mock-begin-date', 'mock-end-date', 100));
    }
    public function testGetQrCodeUrl()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('bizwifi/qrcode/get', ['shop_id' => 100, 'ssid' => 'mock-ssid', 'img_id' => 1])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->getQrCodeUrl(100, 'mock-ssid', 1));
    }
    public function testSetFinishPage()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('bizwifi/finishpage/set', ['foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->setFinishPage(['foo' => 'bar']));
    }
    public function testSetHomePage()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('bizwifi/homepage/set', ['foo' => 'bar'])->andReturn('mock-result');
