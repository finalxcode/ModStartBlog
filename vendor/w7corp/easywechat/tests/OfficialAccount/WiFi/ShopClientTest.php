...
class ShopClientTest extends TestCase
{
    public function testGet()
    {
        ...
        $client->expects()->httpPostJson('bizwifi/shop/get', ['shop_id' => 100])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->get(100));
    }
    public function testList()
    {
        $client = $this->mockApiClient(ShopClient::class);
        $client->expects()->httpPostJson('bizwifi/shop/list', ['pageindex' => 1, 'pagesize' => 20])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->list(1, 20));
    }
    public function testUpdate()
    {
        $client = $this->mockApiClient(ShopClient::class);
        $client->expects()->httpPostJson('bizwifi/shop/update', ['shop_id' => 100, 'foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->update(100, ['foo' => 'bar']));
    }
    public function testClearDevice()
    {
        $client = $this->mockApiClient(ShopClient::class);
        $client->expects()->httpPostJson('bizwifi/shop/clean', ['shop_id' => 100, 'ssid' => 'mock-ssid'])->andReturn('mock-result');
