...
class ClientTest extends TestCase
{
    public function testSend()
    {
        ...
        // to all
        $message = ['filter' => ['is_to_all' => true], 'data' => ['foo' => 'bar']];
        ...
        // to tag
        $message = ['filter' => ['is_to_all' => false, 'tag_id' => 23], 'data' => ['foo' => 'bar']];
        $c->expects()->httpPostJson('cgi-bin/message/mass/sendall', $message)->andReturn('mock-result');
        $this->assertSame('mock-result', $c->send($message));
        // to users
        $message = ['touser' => ['openid1', 'openid2'], 'data' => ['foo' => 'bar']];
    ...
    public function testDelete()
    {
        $c = $this->mockApiClient(Client::class);
        $msgId = 'mock-msg-id';
        $c->expects()->httpPostJson('cgi-bin/message/mass/delete', ['msg_id' => $msgId, 'article_idx' => 0])->andReturn('mock-result');
    ...
    public function testStatus()
    {
        $c = $this->mockApiClient(Client::class);
        $msgId = 'mock-msg-id';
        $c->expects()->httpPostJson('cgi-bin/message/mass/get', ['msg_id' => $msgId])->andReturn('mock-result');
    ...
    public function testPreviewMessage()
    {
        ...
        // to openid
        $c->expects()->preview(['touser' => 'mock-openid', 'msgtype' => 'text', 'text' => ['content' => 'hello world!']])->andReturn('mock-result');
        $this->assertSame('mock-result', $c->previewMessage(new Text('hello world!'), 'mock-openid'));
        // to name
        $c->expects()->preview(['towxname' => 'overtrue', 'msgtype' => 'text', 'text' => ['content' => 'hello world!']])->andReturn('mock-result');
    ...
    public function testSendMessage()
    {
        ...
        // to all
        $c->expects()->send(['filter' => ['is_to_all' => true], 'msgtype' => 'text', 'text' => ['content' => 'hello world!']])->andReturn('mock-result');
        $c->sendMessage(new Text('hello world!'));
        // to tag id
        $c->expects()->send(['filter' => ['is_to_all' => false, 'tag_id' => 12], 'msgtype' => 'text', 'text' => ['content' => 'hello world!']])->andReturn('mock-result');
        $c->sendMessage(new Text('hello world!'), 12);
        // to openids
        $c->expects()->send(['touser' => ['openid1', 'openid2'], 'msgtype' => 'text', 'text' => ['content' => 'hello world!']])->andReturn('mock-result');
