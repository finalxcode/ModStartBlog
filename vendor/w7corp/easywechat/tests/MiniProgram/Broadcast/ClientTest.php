...
class ClientTest extends TestCase
{
    public function testCreate()
    {
        ...
        $params = ['coverImgUrl' => 'foo', 'name' => 'bar', 'priceType' => 1, 'price' => 1.0, 'price2' => 2.0, 'url' => 'pages/goods/index.html?id=10'];
    ...
    {
        $params = ['auditId' => '123456', 'goodsId' => 1];
        $client = $this->mockApiClient(Client::class);
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $params = ['coverImgUrl' => 'foo', 'name' => 'bar', 'priceType' => 1, 'price' => 1.0, 'price2' => 2.0, 'url' => 'pages/goods/index.html?id=10', 'goodsId' => 1];
    ...
    public function testGetPlaybacks()
    {
        ...
        $params = ['action' => 'get_replay', 'room_id' => 1, 'start' => 0, 'limit' => 10];
        $client->expects()->httpPostJson('wxa/business/getliveinfo', $params)->andReturn('mock-result');
    ...
    public function testCreateLiveRoom()
    {
        ...
        $params = [
            ...
            // 房间名字
            'coverImg' => "xxxxxx",
            // 填写mediaID，直播间背景图，图片规则：建议像素1080*1920，大小不超过2M
            'startTime' => 1588237130,
            // 直播计划开始时间，1.开播时间需在当前时间10min后，2.开始时间不能在6个月后
            'endTime' => 1588237130,
            // 直播计划结束时间，1.开播时间和结束时间间隔不得短于30min，不得超过12小时
            'anchorName' => "test1",
            // 主播昵称
            'anchorWechat' => "test1",
            //主播微信号，需通过实名认证，否则将报错
            'shareImg' => "xxx",
            // 填写mediaID，直播间分享图，图片规则：建议像素800*640，大小不超过1M
            'type' => 1,
            // 直播类型，1：推流，0：手机直播
            'screenType' => 0,
            // 1：横屏，0：竖屏，自动根据实际视频分辨率调整
            'closeLike' => 0,
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $params = ['id' => 811, 'name' => '测试更新副号1', 'coverImg' => 'hw7zsntcr0rE-RBfBAaF553DqBk-J02UtWsP8VqrUh3tKu3jO_JwEO8n1cWTJ5TN', 'startTime' => 1607443200, 'endTime' => 1607450400, 'anchorName' => '主播昵称11', 'anchorWechat' => 'lintest1', 'shareImg' => 'hw7zsntcr0rE-RBfBAaF553DqBk-J02UtWsP8VqrUh3tKu3jO_JwEO8n1cWTJ5TN', 'closeLike' => 0, 'closeGoods' => 0, 'closeComment' => 0, 'isFeedsPublic' => 0, 'closeReplay' => 0, 'closeShare' => 0, 'closeKf' => 0, 'feedsImg' => 'hw7zsntcr0rE-RBfBAaF553DqBk-J02UtWsP8VqrUh3tKu3jO_JwEO8n1cWTJ5TN'];
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $params = ['roomId' => 6474, 'users' => [['username' => 'foo', 'nickname' => 'bar']]];
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $params = ['role' => 1, 'offset' => 0, 'limit' => 10, 'keyword' => 'foo'];
