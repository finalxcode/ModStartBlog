...
class ClientTest extends TestCase
{
    public function testAdd()
    {
        ...
        $client->expects()->httpPostJson('wxa/addnearbypoi', ['is_comm_nearby' => '1', 'poi_id' => '', 'foo' => 'bar'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->add(['foo' => 'bar']));
    }
    public function testUpdate()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/addnearbypoi', ['is_comm_nearby' => '1', 'poi_id' => 'mock-poi-id', 'foo' => 'bar'])->andReturn('mock-result');
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('wxa/setnearbypoishowstatus', ['poi_id' => 'mock-poi-id', 'status' => 0])->andReturn('mock-result');
