...
class DevClientTest extends TestCase
{
    public function testGetUsers()
    {
        ...
        $client->expects()->httpPostJson('wxa/devplugin', ['action' => 'dev_apply_list', 'page' => 1, 'num' => 10])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->getUsers());
    }
    public function testAgree()
    {
        $client = $this->mockApiClient(DevClient::class);
        $client->expects()->httpPostJson('wxa/devplugin', ['action' => 'dev_agree', 'appid' => 'mock-appid'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->agree('mock-appid'));
    }
    public function testRefuse()
    {
        $client = $this->mockApiClient(DevClient::class);
        $client->expects()->httpPostJson('wxa/devplugin', ['action' => 'dev_refuse', 'reason' => 'mock-reason'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->refuse('mock-reason'));
    }
    public function testDelete()
    {
        $client = $this->mockApiClient(DevClient::class);
        $client->expects()->httpPostJson('wxa/devplugin', ['action' => 'dev_delete'])->andReturn('mock-result');
