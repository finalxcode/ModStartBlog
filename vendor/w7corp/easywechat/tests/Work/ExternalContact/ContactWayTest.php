...
class ContactWayTest extends TestCase
{
    public function testCreate()
    {
        ...
        $params = ['type' => 1, 'scene' => 2, 'style' => 1, 'state' => 'test-state', 'user' => ['UserID1', 'UserID2', 'UserID3']];
        $client->expects()->httpPostJson('cgi-bin/externalcontact/add_contact_way', $params)->andReturn('mock-result');
        $this->assertSame('mock-result', $client->create(1, 2, ['style' => 1, 'state' => 'test-state', 'user' => ['UserID1', 'UserID2', 'UserID3']]));
    }
    ...
    public function testUpdate()
    {
        $client = $this->mockApiClient(ContactWayClient::class);
        $configId = '42b34949e138eb6e027c123cba77fad7';
        $params = ['config_id' => $configId, 'style' => 1, 'state' => 'test-state', 'user' => ['UserID1', 'UserID2', 'UserID3']];
        $client->expects()->httpPostJson('cgi-bin/externalcontact/update_contact_way', $params)->andReturn('mock-result');
        $this->assertSame('mock-result', $client->update($configId, ['style' => 1, 'state' => 'test-state', 'user' => ['UserID1', 'UserID2', 'UserID3']]));
    }
