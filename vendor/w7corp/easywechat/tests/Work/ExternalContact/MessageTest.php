...
class MessageTest extends TestCase
{
    public function testSubmit()
    {
        ...
        $this->assertSame('mock-result', $client->submit(['external_userid' => ['mock-userid-1', 'mock-userid-2'], 'sender' => 'zhangsan', 'text' => ['content' => 'mock-content'], 'image' => ['media_id' => 'mock-media_id'], 'link' => ['title' => 'mock-title', 'picurl' => 'mock-picurl', 'desc' => 'mock-desc', 'url' => 'mock-url'], 'miniprogram' => ['title' => 'mock-title', 'pic_media_id' => 'mock-pic_media_id', 'appid' => 'mock-appid', 'page' => 'mock-page']]));
    }
    ...
    public function testSubmitWithoutMiniprogramField()
    {
        ...
        $client->shouldReceive('httpPostJson')->andReturn('mock-result');
        try {
            $client->submit(['miniprogram' => ['test']]);
        } catch (\Exception $exception) {
            $this->assertInstanceOf(InvalidArgumentException::class, $exception);
            $this->assertSame('Attribute "title" can not be empty!', $exception->getMessage());
        }
        try {
            $client->submit(['miniprogram' => ['title' => 'mock-title']]);
        } catch (\Exception $exception) {
            $this->assertInstanceOf(InvalidArgumentException::class, $exception);
            $this->assertSame('Attribute "pic_media_id" can not be empty!', $exception->getMessage());
        }
        try {
            $client->submit(['miniprogram' => ['title' => 'mock-title', 'pic_media_id' => 'mock-pic_media_id']]);
        } catch (\Exception $exception) {
            $this->assertInstanceOf(InvalidArgumentException::class, $exception);
            $this->assertSame('Attribute "appid" can not be empty!', $exception->getMessage());
        }
        try {
            $client->submit(['miniprogram' => ['title' => 'mock-title', 'pic_media_id' => 'mock-pic_media_id', 'appid' => 'mock-appid']]);
        } catch (\Exception $exception) {
            $this->assertInstanceOf(InvalidArgumentException::class, $exception);
            $this->assertSame('Attribute "page" can not be empty!', $exception->getMessage());
        }
        $this->assertSame('mock-result', $client->submit(['miniprogram' => ['title' => 'mock-title', 'pic_media_id' => 'mock-pic_media_id', 'appid' => 'mock-appid', 'page' => 'mock-page']]));
    }
    public function testGetGroupMsgResult()
    {
        $client = $this->mockApiClient(MessageClient::class);
        $client->expects()->httpPostJson('cgi-bin/externalcontact/get_group_msg_result', ['msgid' => 'mock-msgid'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->get('mock-msgid'));
    }
    public function testSendWelcome()
    {
        $client = $this->mockApiClient(MessageClient::class);
        $params = ['welcome_code' => 'mock-welcome-code', 'text' => ['content' => 'mock-content']];
        $client->expects()->httpPostJson('cgi-bin/externalcontact/send_welcome_msg', $params)->andReturn('mock-result');
        $this->assertSame('mock-result', $client->sendWelcome('mock-welcome-code', ['text' => ['content' => 'mock-content']]));
    }
}