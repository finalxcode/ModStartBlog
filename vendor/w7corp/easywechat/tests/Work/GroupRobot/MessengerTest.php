...
class MessengerTest extends TestCase
{
    ...
    public function testSendWithText()
    {
        $client = \Mockery::mock(Client::class);
        $messenger = new Messenger($client);
        $client->expects()->send('mock-key', ['msgtype' => 'text', 'text' => ['content' => 'hello', 'mentioned_list' => [], 'mentioned_mobile_list' => []]])->times(2)->andReturn('mock-result');
    ...
    public function testSendWithMention()
    {
        $client = \Mockery::mock(Client::class);
        $messenger = new Messenger($client);
        $client->expects()->send('mock-key', ['msgtype' => 'text', 'text' => ['content' => 'hello', 'mentioned_list' => ['mock-userid', '@all'], 'mentioned_mobile_list' => ['mock-mobile', 'mock-mobile2']]])->times(2)->andReturn('mock-result');
    ...
    public function testSendWithMarkdown()
    {
        $client = \Mockery::mock(Client::class);
        $messenger = new Messenger($client);
        $client->expects()->send('mock-key', ['msgtype' => 'markdown', 'markdown' => ['content' => 'hello']])->andReturn('mock-result');
    ...
    public function testSendWithImage()
    {
        $client = \Mockery::mock(Client::class);
        $messenger = new Messenger($client);
        $client->expects()->send('mock-key', ['msgtype' => 'image', 'image' => ['base64' => 'mock-base64', 'md5' => 'mock-md5']])->andReturn('mock-result');
    ...
    public function testSendWithNews()
    {
        $client = \Mockery::mock(Client::class);
        $messenger = new Messenger($client);
        $client->expects()->send('mock-key', ['msgtype' => 'news', 'news' => ['articles' => [['title' => 'mock-title', 'description' => 'mock-description', 'url' => 'mock-url', 'picurl' => 'mock-picurl']]]])->andReturn('mock-result');
        $message = new News([new NewsItem(['title' => 'mock-title', 'description' => 'mock-description', 'url' => 'mock-url', 'image' => 'mock-picurl'])]);
        $this->assertSame('mock-result', $messenger->message($message)->toGroup('mock-key')->send());
    }
}