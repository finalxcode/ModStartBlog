...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpGet('cgi-bin/user/simplelist', ['department_id' => 14, 'fetch_child' => 0])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->getDepartmentUsers(14));
        $client->expects()->httpGet('cgi-bin/user/simplelist', ['department_id' => 15, 'fetch_child' => 1])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->getDepartmentUsers(15, true));
    }
    public function testGetDetailedDepartmentUsers()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpGet('cgi-bin/user/list', ['department_id' => 18, 'fetch_child' => 0])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->getDetailedDepartmentUsers(18));
        $client->expects()->httpGet('cgi-bin/user/list', ['department_id' => 18, 'fetch_child' => 1])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->getDetailedDepartmentUsers(18, true));
    }
    public function testUserIdToOpenid()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/user/convert_to_openid', ['userid' => 'overtrue', 'agentid' => null])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->userIdToOpenid('overtrue'));
        $client->expects()->httpPostJson('cgi-bin/user/convert_to_openid', ['userid' => 'overtrue', 'agentid' => 39202])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->userIdToOpenid('overtrue', 39202));
    }
    public function testOpenidToUserId()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/user/convert_to_userid', ['openid' => 'mock-openid'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->openidToUserId('mock-openid'));
    }
    public function testMobileToUserId()
    {
        $client = $this->mockApiClient(Client::class);
        $client->expects()->httpPostJson('cgi-bin/user/getuserid', ['mobile' => 'mock-mobile'])->andReturn('mock-result');
