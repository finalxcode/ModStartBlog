...
class TagClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(TagClient::class);
        $client->expects()->httpPostJson('cgi-bin/tag/update', ['tagid' => 12, 'tagname' => '粉丝'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->update(12, '粉丝'));
    }
    public function testDelete()
    {
        $client = $this->mockApiClient(TagClient::class);
        $client->expects()->httpGet('cgi-bin/tag/delete', ['tagid' => 12])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->delete(12));
    }
    public function testGet()
    {
        $client = $this->mockApiClient(TagClient::class);
        $client->expects()->httpGet('cgi-bin/tag/get', ['tagid' => 12])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->get(12));
    }
    public function testTagUsers()
    {
        $client = $this->mockApiClient(TagClient::class);
        $client->expects()->httpPostJson('cgi-bin/tag/addtagusers', ['tagid' => 12, 'userlist' => ['foo', 'bar'], 'partylist' => []])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->tagUsers(12, ['foo', 'bar']));
    }
    public function testUntagUsers()
    {
        $client = $this->mockApiClient(TagClient::class);
        $client->expects()->httpPostJson('cgi-bin/tag/deltagusers', ['tagid' => 12, 'userlist' => ['foo', 'bar'], 'partylist' => []])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->untagUsers(12, ['foo', 'bar']));
    }
    public function testTagDepartments()
    {
        $client = $this->mockApiClient(TagClient::class);
        $client->expects()->httpPostJson('cgi-bin/tag/addtagusers', ['tagid' => 12, 'userlist' => [], 'partylist' => [14, 26]])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->tagDepartments(12, [14, 26]));
    }
    public function testUntagDepartments()
    {
        $client = $this->mockApiClient(TagClient::class);
        $client->expects()->httpPostJson('cgi-bin/tag/deltagusers', ['tagid' => 12, 'userlist' => [], 'partylist' => [14, 26]])->andReturn('mock-result');
