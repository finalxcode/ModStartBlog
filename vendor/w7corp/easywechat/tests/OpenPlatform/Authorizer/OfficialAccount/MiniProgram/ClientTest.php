...
class ClientTest extends TestCase
{
    ...
    {
        $client = $this->mockApiClient(Client::class, [], new ServiceContainer(['app_id' => 'app-id']));
        $client->expects()->httpPostJson('cgi-bin/wxopen/wxamplink', ['appid' => 'wxa', 'notify_users' => false, 'show_profile' => true])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->link('wxa', false, true));
    }
    public function testUnlink()
    {
        $client = $this->mockApiClient(Client::class, [], new ServiceContainer(['app_id' => 'app-id']));
        $client->expects()->httpPostJson('cgi-bin/wxopen/wxampunlink', ['appid' => 'wxa'])->andReturn('mock-result');
