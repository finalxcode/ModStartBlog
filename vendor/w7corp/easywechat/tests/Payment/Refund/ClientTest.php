...
class ClientTest extends TestCase
{
    ...
    public function testByOutTradeNumber()
    {
        ...
        $refundFee = 1;
        $optional = ['foo' => 'bar'];
        $params = array_merge(['out_trade_no' => $orderNo, 'out_refund_no' => $refundNo, 'total_fee' => $totalFee, 'refund_fee' => $totalFee, 'appid' => 'wx123456'], $optional);
    ...
    public function testByTransactionId()
    {
        ...
        $refundFee = 1;
        $optional = ['foo' => 'bar'];
        $params = array_merge(['transaction_id' => $orderNo, 'out_refund_no' => $refundNo, 'total_fee' => $totalFee, 'refund_fee' => $totalFee, 'appid' => 'wx123456'], $optional);
        $client->expects()->safeRequest('secapi/pay/refund', $params)->andReturn('mock-result');
    ...
    public function testQueryByTransactionId()
    {
        ...
        $client->expects()->request('pay/refundquery', ['transaction_id' => 'foobar', 'appid' => 'wx123456'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->queryByTransactionId('foobar'));
    }
    public function testQueryByOutTradeNumber()
    {
        $client = $this->mockApiClient(Client::class, ['request'], $this->getApp())->makePartial();
        $client->expects()->request('pay/refundquery', ['out_trade_no' => 'foobar', 'appid' => 'wx123456'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->queryByOutTradeNumber('foobar'));
    }
    public function testQueryByOutRefundNumber()
    {
        $client = $this->mockApiClient(Client::class, ['request'], $this->getApp())->makePartial();
        $client->expects()->request('pay/refundquery', ['out_refund_no' => 'foobar', 'appid' => 'wx123456'])->andReturn('mock-result');
        $this->assertSame('mock-result', $client->queryByOutRefundNumber('foobar'));
    }
    public function testQueryByRefundId()
    {
        $client = $this->mockApiClient(Client::class, ['request'], $this->getApp())->makePartial();
        $client->expects()->request('pay/refundquery', ['refund_id' => 'foobar', 'appid' => 'wx123456'])->andReturn('mock-result');
