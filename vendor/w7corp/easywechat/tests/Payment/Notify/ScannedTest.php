...
class ScannedTest extends TestCase
{
    ...
    {
        return new Application(array_merge(['app_id' => 'mock-app-id', 'mch_id' => 'mock-mch-id', 'key' => '88888888888888888888888888888888'], $config));
    }
...
<sign>280F9CB28E99DC917792FCC7AC1B88C4</sign>
        ...
        $response = $notify->handle(function ($message) use($that) {
            $that->assertSame(['foo' => 'bar', 'sign' => '280F9CB28E99DC917792FCC7AC1B88C4'], $message);
            return 'prepay-id';
        });
        // return prepay_id.
        ...
        $this->assertArraySubset(['return_code' => 'FAIL', 'return_msg' => 'something-went-wrong...', 'result_code' => 'FAIL', 'err_code_des' => null], XML::parse($response->getContent()));
    }
...
<foo>bar</foo>
        ...
        $this->assertArraySubset(['return_code' => 'SUCCESS', 'return_msg' => null, 'result_code' => 'FAIL', 'err_code_des' => 'sold out!'], XML::parse($response->getContent()));
    }
}