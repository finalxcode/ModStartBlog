...
class PaidTest extends TestCase
{
    ...
    {
        return new Application(array_merge(['key' => '88888888888888888888888888888888'], $config));
    }
...
<sign>834A25C9A5B48305AB997C9A7E101530</sign>
        ...
        $response = $notify->handle(function ($message) use($that) {
            $that->assertSame(['foo' => 'bar', 'sign' => '834A25C9A5B48305AB997C9A7E101530'], $message);
            return true;
        });
        // return true.
        $this->assertInstanceOf(Response::class, $response);
        $this->assertSame(['return_code' => 'SUCCESS', 'return_msg' => null], XML::parse($response->getContent()));
        // return false.
        $response = $notify->handle(function () {
            return false;
        });
        $this->assertSame(['return_code' => 'FAIL', 'return_msg' => null], XML::parse($response->getContent()));
        // empty return.
        $response = $notify->handle(function () {
        });
        $this->assertSame(['return_code' => 'FAIL', 'return_msg' => null], XML::parse($response->getContent()));
        ...
        $this->assertSame(['return_code' => 'FAIL', 'return_msg' => 'fails.'], XML::parse($response->getContent()));
    }
