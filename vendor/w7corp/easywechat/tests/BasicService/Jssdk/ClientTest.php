...
class ClientTest extends TestCase
{
    ...
    public function testSignature()
    {
        ...
        $client->allows()->getTicketSignature('mock-ticket', \Mockery::type('string'), \Mockery::type('integer'), $url)->andReturn('mock-signature');
    ...
    public function testGetTicketSignature()
    {
        ...
        $timestamp = time();
        $url = 'http://easywechat.com';
        $this->assertSame(sha1("jsapi_ticket={$ticket}&noncestr={$nonce}&timestamp={$timestamp}&url={$url}"), $client->getTicketSignature($ticket, $nonce, $timestamp, $url));
    }
