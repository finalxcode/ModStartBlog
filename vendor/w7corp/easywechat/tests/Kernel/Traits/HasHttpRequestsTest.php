...
class HasHttpRequestsTest extends TestCase
{
    ...
    {
        $this->assertSame(['curl' => [CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4]], HasHttpRequests::getDefaultOptions());
    ...
    public function testRequest()
    {
        ...
        // default arguments
        $client->expects()->request('GET', 'foo/bar', ['curl' => [CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4], 'handler' => $handlerStack, 'base_uri' => 'http://easywechat.com'])->andReturn($response);
        $this->assertSame($response, $cls->request('foo/bar'));
        // custom arguments
        $client->expects()->request('POST', 'foo/bar', ['curl' => [CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4], 'query' => ['foo' => 'bar'], 'handler' => $handlerStack, 'base_uri' => 'http://easywechat.com'])->andReturn($response);
    ...
    public function testFixJsonIssue()
    {
        ...
        $response = new Response(200, [], 'mock-result');
        // default arguments
        $client->expects()->request('POST', 'foo/bar', ['curl' => [CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4], 'handler' => $handlerStack, 'body' => '{}', 'headers' => ['Content-Type' => 'application/json'], 'base_uri' => 'http://easywechat.com'])->andReturn($response);
        $this->assertSame($response, $cls->request('foo/bar', 'POST', ['json' => []]));
        // unescape unicode
        $client->expects()->request('POST', 'foo/bar', ['curl' => [CURLOPT_IPRESOLVE => CURL_IPRESOLVE_V4], 'handler' => $handlerStack, 'body' => '{"name":"中文"}', 'headers' => ['Content-Type' => 'application/json'], 'base_uri' => 'http://easywechat.com'])->andReturn($response);
        $cls->request('foo/bar', 'POST', ['json' => ['name' => '中文']]);
    }
}
