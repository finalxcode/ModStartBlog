...
class ServerGuardTest extends TestCase
{
    public function testServe()
    {
        ...
        $logger->expects()->debug('Request received:', ['method' => 'POST', 'uri' => 'http://localhost/path/to/resource?foo=bar', 'content-type' => 'xml', 'content' => '<xml><name>foo</name></xml>']);
        $request = Request::create('/path/to/resource?foo=bar', 'POST', ['foo' => 'bar'], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><name>foo</name></xml>');
        $app = new ServiceContainer(['token' => 'mock-token'], ['logger' => $logger, 'request' => $request]);
    ...
    public function testValidate()
    {
        $time = time();
        $nonce = 'foobar';
        $params = ['mock-token', $time, $nonce];
        sort($params, SORT_STRING);
        $signature = sha1(implode($params));
        // with signature
        $request = Request::create('/path/to/resource?foo=bar', 'POST', ['timestamp' => $time, 'nonce' => $nonce, 'signature' => $signature, 'encrypt_type' => 'aes'], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><name>foo</name></xml>');
        $app = new ServiceContainer(['token' => 'mock-token'], ['request' => $request]);
    ...
    public function testValidateWithInvalidSignature()
    {
        ...
        $signature = sha1(implode($params));
        // with signature
        $request = Request::create('/path/to/resource?foo=bar', 'POST', ['timestamp' => $time, 'nonce' => $nonce, 'encrypt_type' => 'aes', 'signature' => $signature . 'xxxx'], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><name>foo</name></xml>');
    ...
    public function testValidateWithoutSignature()
    {
        ...
        $nonce = 'foobar';
        // without signature
        $request = Request::create('/path/to/resource?foo=bar', 'POST', ['timestamp' => $time, 'nonce' => $nonce], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><name>foo</name></xml>');
        $app = new ServiceContainer([], ['request' => $request]);
        $guard = new ServerGuard($app);
        $this->assertSame($guard, $guard->validate());
    }
    ...
    {
        $request = Request::create('/path/to/resource?foo=bar', 'POST', [], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><Content>foo</Content><MsgType>text</MsgType></xml>');
        $app = new ServiceContainer([], ['request' => $request]);
        $guard = new ServerGuard($app);
    ...
    public function testGetMessageInSafeMode()
    {
        ...
        // xml
        $request = Request::create('/path/to/resource?foo=bar', 'POST', ['nonce' => $nonce, 'timestamp' => $time, 'signature' => $signature, 'msg_signature' => 'mock-msg-signature', 'encrypt_type' => 'aes'], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><Encrypt>encrypted content</Encrypt></xml>');
        $encryptor->allows()->decrypt('encrypted content', 'mock-msg-signature', $nonce, $time)->andReturn(XML::build(['foo' => 'bar']));
        $app = new ServiceContainer([], ['request' => $request, 'encryptor' => $encryptor]);
        $guard = new ServerGuard($app);
        $this->assertSame(['foo' => 'bar'], $guard->getMessage());
        // json
        $request = Request::create('/path/to/resource?foo=bar', 'POST', ['nonce' => $nonce, 'timestamp' => $time, 'signature' => $signature, 'msg_signature' => 'mock-msg-signature2', 'encrypt_type' => 'aes'], [], [], ['CONTENT_TYPE' => ['application/json']], '<xml><Encrypt>encrypted content</Encrypt></xml>');
        $encryptor->allows()->decrypt('encrypted content', 'mock-msg-signature2', $nonce, $time)->andReturn(json_encode(['foo' => 'bar']));
        $app = new ServiceContainer([], ['request' => $request, 'encryptor' => $encryptor]);
        $guard = new ServerGuard($app);
        $this->assertSame(['foo' => 'bar'], $guard->getMessage());
    }
    ...
    {
        $request = Request::create('/path/to/resource?foo=bar', 'POST', [], [], [], ['CONTENT_TYPE' => ['application/xml']], 'not-xml-content');
        $app = new ServiceContainer([], ['request' => $request]);
        $guard = new ServerGuard($app);
    ...
    {
        $request = Request::create('/path/to/resource?foo=bar', 'POST', [], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml></xml>');
        $app = new ServiceContainer([], ['request' => $request]);
        $guard = new ServerGuard($app);
        $this->expectException(BadRequestException::class);
    ...
    {
        $request = Request::create('/path/to/resource', 'POST', [], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><foo>bar</foo></xml>');
        $app = new ServiceContainer([], ['request' => $request]);
        ...
        $guard->allows()->handleRequest()->andReturn(['to' => 'overtrue', 'from' => 'easywechat', 'response' => 'hello overtrue!']);
    ...
    public function testBuildResponse()
    {
        ...
        $this->assertSame('welcome to easywechat.com', $response['Content']);
        // news
        $item1 = new NewsItem(['title' => 'mock-title-1', 'description' => 'mock-description-1', 'url' => 'mock-url-1', 'image' => 'mock-image-1']);
        $item2 = new NewsItem(['title' => 'mock-title-2', 'description' => 'mock-description-2', 'url' => 'mock-url-2', 'image' => 'mock-image-2']);
        ...
        $request = Request::create('/path/to/resource?foo=bar', 'POST', ['timestamp' => $time, 'nonce' => $nonce, 'signature' => $signature, 'encrypt_type' => 'aes'], [], [], ['CONTENT_TYPE' => ['application/xml']], '<xml><Content>foo</Content><MsgType>text</MsgType></xml>');
        $encryptor = \Mockery::mock(Encryptor::class);
        ...
        $app = new ServiceContainer([], ['logger' => $logger, 'request' => $request, 'encryptor' => $encryptor]);
        $guard = new ServerGuard($app);
    ...
    public function testHandleRequest()
    {
        ...
        // no message type
        $message = ['FromUserName' => 'overtrue', 'ToUserName' => 'easywechat'];
        ...
        $this->assertSame(['to' => 'overtrue', 'from' => 'easywechat', 'response' => 'mock-response'], $guard->handleRequest());
        // with message type
        $message = ['FromUserName' => 'overtrue', 'ToUserName' => 'easywechat', 'MsgType' => 'image'];
        $guard->expects()->getMessage()->andReturn($message);
    ...
    {
        $timestamp = time();
        $xml = (new Text('hello world!'))->transformToXml(['ToUserName' => 'overtrue', 'FromUserName' => 'easywechat', 'MsgType' => 'text', 'CreateTime' => $timestamp]);
        $request = Request::create('/path/to/resource?foo=bar', 'POST', []);
        $app = new ServiceContainer(['app_id' => 'appId', 'token' => 'mock-token'], ['request' => $request]);
        $guard = \Mockery::mock(ServerGuard::class, [$app])->makePartial();
        $this->assertSame(['MsgType' => 'text', 'Content' => 'hello world!', 'ToUserName' => 'overtrue', 'FromUserName' => 'easywechat', 'CreateTime' => strval($timestamp)], $guard->parseMessage($xml));
        // json format
        $this->assertSame(['MsgType' => 'text', 'Content' => 'hello world!', 'ToUserName' => 'overtrue', 'FromUserName' => 'easywechat', 'CreateTime' => strval($timestamp)], $guard->parseMessage(json_encode(XML::parse($xml))));
    }
