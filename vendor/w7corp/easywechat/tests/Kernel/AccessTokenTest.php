...
class AccessTokenTest extends TestCase
{
    ...
    public function testGetToken()
    {
        ...
        $token = \Mockery::mock(AccessToken::class . '[getCacheKey,getCache,requestToken,setToken,getCredentials]', [new ServiceContainer()])->shouldAllowMockingProtectedMethods();
        $credentials = ['foo' => 'foo', 'bar' => 'bar'];
        ...
        $tokenResult = ['access_token' => 'mock-cached-token', 'expires_in' => 7200];
    ...
    public function testSetToken()
    {
        ...
        $cache = \Mockery::mock(CacheInterface::class);
        $token = \Mockery::mock(AccessToken::class . '[getCacheKey,getCache]', [$app])->shouldAllowMockingProtectedMethods();
        $token->allows()->getCacheKey()->andReturn('mock-cache-key');
        $token->allows()->getCache()->andReturn($cache);
        $cache->expects()->has('mock-cache-key')->andReturn(true);
        $cache->expects()->set('mock-cache-key', ['access_token' => 'mock-token', 'expires_in' => 7200], 7200)->andReturn(true);
        $result = $token->setToken('mock-token');
        $this->assertSame($token, $result);
        // 7000
        $cache->expects()->has('mock-cache-key')->andReturn(true);
        $cache->expects()->set('mock-cache-key', ['access_token' => 'mock-token', 'expires_in' => 7000], 7000)->andReturn(true);
        $result = $token->setToken('mock-token', 7000);
        $this->assertSame($token, $result);
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('Failed to cache access token.');
        $cache->expects()->has('mock-cache-key')->andReturn(false);
        $cache->expects()->set('mock-cache-key', ['access_token' => 'mock-token', 'expires_in' => 7000], 7000)->andReturn(false);
        $token->setToken('mock-token', 7000);
    }
    public function testRefresh()
    {
        $app = \Mockery::mock(ServiceContainer::class);
        $token = \Mockery::mock(AccessToken::class . '[getToken]', [$app])->shouldAllowMockingProtectedMethods();
    ...
    {
        $app = new ServiceContainer(['response_type' => 'collection']);
        $token = \Mockery::mock(AccessToken::class . '[sendRequest]', [$app])->shouldAllowMockingProtectedMethods();
        $credentials = ['foo' => 'foo', 'bar' => 'bar'];
        // succeed
    ...
    {
        $app = \Mockery::mock(ServiceContainer::class);
        $token = \Mockery::mock(AccessToken::class . '[getQuery]', [$app])->shouldAllowMockingProtectedMethods();
    ...
    {
        $app = \Mockery::mock(ServiceContainer::class)->makePartial();
        $token = \Mockery::mock(AccessToken::class . '[setHttpClient,request,getEndpoint,sendRequest]', [$app])->shouldAllowMockingProtectedMethods();
        $credentials = ['appid' => '123', 'secret' => 'pa33w0rd'];
        ...
        $token->expects()->request('/auth/get-token', 'GET', ['query' => $credentials])->andReturn(new Response(200, [], 'mock-response'));
        $response = $token->sendRequest($credentials);
        $this->assertSame('mock-response', $response->getBody()->getContents());
        // sub instance
        $token = \Mockery::mock(DummyAccessTokenForTest::class . '[setHttpClient,request,getEndpoint,sendRequest]', [$app])->shouldAllowMockingProtectedMethods();
        $credentials = ['appid' => '123', 'secret' => 'pa33w0rd'];
        $token->expects()->sendRequest($credentials)->passthru();
        $token->expects()->getEndpoint()->andReturn('/auth/get-token');
        $token->expects()->setHttpClient($app['http_client'])->andReturnSelf();
        $token->expects()->request('/auth/get-token', 'post', ['json' => $credentials])->andReturn(new Response(200, [], 'mock-response'));
        $response = $token->sendRequest($credentials);
        $this->assertSame('mock-response', $response->getBody()->getContents());
    }
    ...
    {
        $app = \Mockery::mock(ServiceContainer::class)->makePartial();
        $token = \Mockery::mock(AccessToken::class . '[getCacheKey,getCredentials]', [$app])->shouldAllowMockingProtectedMethods();
        $credentials = ['appid' => '123', 'secret' => 'pa33w0rd'];
        $token->allows()->getCredentials()->andReturn($credentials);
    ...
    {
        $app = \Mockery::mock(ServiceContainer::class)->makePartial();
        $token = \Mockery::mock(AccessToken::class . '[getToken,getQuery]', [$app])->shouldAllowMockingProtectedMethods();
        ...
        // sub instance
        $token = \Mockery::mock(DummyAccessTokenForTest::class . '[getToken,getQuery]', [$app])->shouldAllowMockingProtectedMethods();
...
class DummyAccessTokenForTest extends AccessToken
{
    ...
    {
        return ['appid' => 1234, 'secret' => 'pas33w0rd'];
    }
}