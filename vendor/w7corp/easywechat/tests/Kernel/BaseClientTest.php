...
class BaseClientTest extends TestCase
{
    public function makeClient($methods = [], ServiceContainer $app = null, AccessToken $accessToken = null)
    {
        ...
        $app = isset($app) ? $app : \Mockery::mock(ServiceContainer::class);
        $accessToken = isset($accessToken) ? $accessToken : \Mockery::mock(AccessToken::class, [$app]);
    ...
    public function testRequest()
    {
        ...
        $client = $this->makeClient(['registerHttpMiddlewares', 'performRequest'], $app)->shouldAllowMockingProtectedMethods();
        // default value
    ...
    public function testHttpClient()
    {
        ...
        $app = new ServiceContainer([], ['http_client' => $http]);
        $client = $this->makeClient('request', $app);
        $this->assertSame($http, $client->getHttpClient());
    }
    public function testRegisterMiddlewares()
    {
        $client = $this->makeClient(['retryMiddleware', 'accessTokenMiddleware', 'logMiddleware', 'pushMiddleware'])->shouldAllowMockingProtectedMethods()->makePartial();
    ...
    public function testAccessTokenMiddleware()
    {
        ...
        $client = $this->makeClient(['accessTokenMiddleware'], $app, $accessToken)->shouldAllowMockingProtectedMethods()->makePartial();
    ...
    {
        $app = new ServiceContainer(['http' => ['log_template']]);
        $app['logger'] = new Logger('logger');
        $client = $this->makeClient(['accessTokenMiddleware'], $app)->shouldAllowMockingProtectedMethods()->makePartial();
    ...
    public function testRetryMiddleware()
    {
        ...
        $client = $this->makeClient(['retryMiddleware'], $app, $accessToken)->shouldAllowMockingProtectedMethods()->makePartial();
        ...
        $handler = new MockHandler([new Response(200, [], '{"errcode":40001}'), new Response(200, [], '{"success": true}')]);
        ...
        $logger->expects()->debug('Retrying with refreshed access token.');
        $accessToken->expects()->refresh();
        $handler = new MockHandler([new Response(200, [], '{"errcode":40001}'), new Response(200, [], '{"errcode":42001}'), new Response(200, [], '{"success": true}')]);
        $handler = $func($handler);
        $c = new Client(['handler' => $handler]);
        $p = $c->sendAsync(new Request('GET', 'http://easywechat.com'), []);
        $response = $p->wait();
        ...
        // 3 times
        $app['config']['http'] = ['max_retries' => 3, 'retry_delay' => 1];
        $logger->expects()->debug('Retrying with refreshed access token.')->times(3);
        $accessToken->expects()->refresh()->times(3);
        $handler = new MockHandler([new Response(200, [], '{"errcode":40001}'), new Response(200, [], '{"errcode":42001}'), new Response(200, [], '{"errcode":40001}'), new Response(200, [], '{"success":true}')]);
        $handler = $func($handler);
        $c = new Client(['handler' => $handler]);
        $s = microtime(true);
        $p = $c->sendAsync(new Request('GET', 'http://easywechat.com'), []);
        $response = $p->wait();
        $this->assertTrue(microtime(true) - $s >= 3 * ($app['config']['http']['retry_delay'] / 1000), 'delay time');
        // times * delay
        $this->assertSame(200, $response->getStatusCode());
        $this->assertSame('{"success":true}', $response->getBody()->getContents());
    }
