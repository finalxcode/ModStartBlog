...
class ClientTest extends TestCase
{
    ...
    {
        $app = new ServiceContainer(['corp_id' => 'mock-corp-id', 'redirect_uri_single' => 'mock-redirect-uri']);
        $app['provider_access_token'] = \Mockery::mock(AccessToken::class);
        $client = $this->mockApiClient(Client::class, [], $app);
    ...
    public function testGetLoginInfo()
    {
        ...
        $client->expects()->httpPostJson('cgi-bin/service/get_login_info', ['auth_code' => 'mock-auth-code'])->andReturn('mock-result');
    ...
    public function testGetRegisterUriWithDefaultParam()
    {
        ...
        $client->expects()->getRegisterCode()->andReturn(['register_code' => 'mock-register-code']);
    ...
    {
        $app = new ServiceContainer(['corp_id' => 'mock-corp-id', 'reg_template_id' => 'mock-reg-template-id']);
        $app['provider_access_token'] = \Mockery::mock(AccessToken::class);
        $client = $this->mockApiClient(Client::class, [], $app)->makePartial();
        $client->expects()->httpPostJson('cgi-bin/service/get_register_code', ['template_id' => 'mock-reg-template-id', 'corp_name' => 'mock-corp-name', 'admin_name' => 'mock-admin-name', 'admin_mobile' => 'mock-admin-mobile', 'state' => 'mock-state'])->andReturn('mock-result');
    ...
    public function testGetRegisterCodeWithDefaultParam()
    {
        ...
        $app['provider_access_token'] = \Mockery::mock(AccessToken::class);
        $client = $this->mockApiClient(Client::class, [], $app)->makePartial();
        $client->expects()->httpPostJson('cgi-bin/service/get_register_code', ['template_id' => 'mock-reg-template-id'])->andReturn('mock-result');
    ...
    public function testGetRegisterInfo()
    {
        ...
        $app['provider_access_token'] = \Mockery::mock(AccessToken::class);
        $client = $this->mockApiClient(Client::class, [], $app)->makePartial();
        $client->expects()->httpPostJson('cgi-bin/service/get_register_info', ['register_code' => 'mock-register-code'])->andReturn('mock-result');
    ...
    public function testSetAgentScope()
    {
        ...
        $app['provider_access_token'] = \Mockery::mock(AccessToken::class);
        $client = $this->mockApiClient(Client::class, [], $app)->makePartial();
        $client->expects()->httpGet('cgi-bin/agent/set_scope', ['agentid' => 'mock-agent-id', 'allow_user' => ['user1', 'user2'], 'allow_party' => ['party1', 'party2'], 'allow_tag' => ['tag1', 'tag2'], 'access_token' => 'mock-access-token'])->andReturn('mock-result');
    ...
    public function testContactSyncSuccess()
    {
        ...
        $app['provider_access_token'] = \Mockery::mock(AccessToken::class);
        $client = $this->mockApiClient(Client::class, [], $app)->makePartial();
        $client->expects()->httpGet('cgi-bin/sync/contact_sync_success', ['access_token' => 'mock-access-token'])->andReturn('mock-result');
    ...
    public function testSearchContact()
    {
        ...
        $client->expects()->httpPostJson('cgi-bin/service/contact/search', [
            ...
            'query_word' => 'user',
            'query_type' => 1,
            // 1：查询用户，返回用户userid列表 2：查询部门，返回部门id列表。 不填该字段或者填0代表同时查询部门跟用户
