...
class ApplicationTest extends TestCase
{
    ...
    {
        $app = new Application(['mch_id' => 'foo-merchant-id']);
    ...
    {
        $app = new Application(['key' => '88888888888888888888888888888888']);
        $this->assertTrue($app->verifySignature(['foo' => 'bar', 'sign' => 'A17558A18A547578818256FD52E74BB3EA9475E021F5F210809F2B7916C53B1D']));
        $this->assertTrue($app->verifySignature(['foo' => 'bar', 'sign' => '834A25C9A5B48305AB997C9A7E101530']));
        $this->assertFalse($app->verifySignature(['foo' => 'bar']));
        $this->expectException(InvalidSignException::class);
        $this->expectExceptionMessage('return value signature verification error');
        $this->assertTrue($app->verifySignature(['foo' => 'bar', 'sign' => '834A25C9A5B48305AB997C9A7E101531']));
    }
}